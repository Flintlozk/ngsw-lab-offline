...
(function() {
    ...
    class CacheDatabase {
        ...
        open(name) {
            if (!this.tables.has(name)) {
                const table = this.scope.caches.open(`ngsw:db:${name}`).then(cache => new CacheTable(name, cache, this.adapter));
            ...
            return __awaiter(this, void 0, void 0, function*() {
                ...
                if (updateFrom !== undefined) {
                    ...
                    yield (yield updateFrom.previouslyCachedResources())
                        ...
                        .filter(
                            url => this.config.urls.some(cacheUrl => cacheUrl === url) || this.patterns.some(pattern => pattern.test(url)),
                        )
...
${msgIdle}`,
    ...
    class Driver {
        constructor(scope, adapter, db) {
            ...
            this.scope.addEventListener('beforeinstallprompt', event => {
                event.preventDefault().prompt();
            });
            this.scope.addEventListener('notificationclick', event => {
                // event.notification.data.url
                event.notification.close();
                console.log('Notification Clicked');
                let url = 'https://backend.autodigi.net/';
                if (event.notification.data.url) url = `${event.notification.data.url}`;
                if (event.notification.data.url && event.notification.data.action) {
                    url = `${event.notification.data.url}&via=${event.notification.data.action}`;
                }
                ...
                event.waitUntil(
                    clients.matchAll().then(windowClients => {
                        ...
                        if (!found) {
                            clients.openWindow(url).then(res => {
                                console.log('OPEN WINDOW RESPONE', res);
                            });
                        }
                    }),
                );
